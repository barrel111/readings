...
#remark[Technically, $O(dot)$ represents a set of functions. Still, we may write an equation involving $O(dot)$ (for eg. $f = O(g)$) in which case $O(dot)$ just represents some function from this asymptotic class. This remark holds for all the other asymptotic classes we will define. ]
