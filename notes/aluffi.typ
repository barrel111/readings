...
#prop("Canonical Decomposition")[Let $f: A to B$ be any function and define $tilde$ as above. Then $f$ decomposes as follows:
#v(15pt)
#align(center)[
  #commutative-diagram(
    node-padding: (45pt, 50pt),
...
- for every two objects $A, B$ of $sans(C)$, a set $"Hom"_(sans(C)) (A, B)$ of _morphisms_ with the following properties
  ...
  - the sets $"Hom"_(sans(C)) (A, B)$ and $"Hom"_(sans(C)) (C, D)$ are disjoint unless $A = C$ and $B = D$.]

#definition[A morphism of an object $A$ of a category $C$ to itself is called an _endomorphism_. Furthermore, $"Hom"_(sans(C))(A, A)$ is also denoted $"End"_C(A)$.]

#definition[A _diagram commutes_ if all ways to traverse it lead to the same results of composing morphisms along the way.]

#example($sans("Set")$)[By $sans("Set")$ we denote the category of sets, where
- $"Obj"(sans("Set")) = $ the class of all sets;
- for $A, B$ in $"Obj"(sans("Set"))$, $"Hom"_(sans("Set"))(A, B) = B^A.$
Composition of morphisms is defined to be the same as the composition of set-functions. ]

#example("Relations")[Suppose $tilde$ is a reflexive and transitive relation on some set $S$. Then, we can encode this data into a category.
- _objects_: the elements of $S$;
- _morphisms_: if $a, b$ are objects, then let $Hom("", a, b)$ be the set consisting of the element $(a, b) in S times S$ if $a tilde b$ and let $Hom("", a, b) = emptyset$ otherwise.
For composition, let $a, b, c$ be objects and $f in Hom("", a, b)$ and $g in Hom("", b, c)$. Then, $g f in Hom("", a, c)$ is defined to be $ g f = (a, c). $ This is a _small category_. ]

#let Shat = $accent(sans(S), hat)$
#example("Partial Ordering of Sets")[Let $S$ be a set. Define another (_small_) category $Shat$ by
- $Obj(Shat) = cal(P)(S)$;
- for $A, B$ objects of $Shat$, let $Hom(Shat, A, B)$ be the pair $(A, B)$ if $A subset.eq B$ and let $Hom(Shat, A, B) = emptyset$ otherwise.
For composition, let $A, B, C$ be objects and $f in Hom(Shat, A, B)$ and $g in Hom(Shat, B, C)$. Then, $g f in Hom(Shat, A, C)$ is definted to be $ g f = (A, C). $]

#example("Slice Category")[Let $sans(C)$ be a category and let $A$ be an object of $sans(C)$. We will define a category $sans(C)_A$ whose objects are certain _morphisms_ in $sans(C)$ and whose morphisms are certain _diagrams_ of $sans(C)$.
- $Obj(sans(C)_A) = $ all morphisms from any object of $sans(C)$ to $A$; that is, an object of $sans(C)$ is an element $f in Hom(sans(C), Z, A)$ for some object $Z$ of $sans(C)$. Pictorally, an object of $sans(C)_A$ is an arrow $Z ->^f A$ _in_ $sans(C)$;
 #align(center)[
    #commutative-diagram(
...
- for objects $f_1, f_2$ of $sans(C)_A$, that is two arrows #align(center)[
    #commutative-diagram(
      node-padding: (30pt, 40pt),
      node((0, 0), $Z_1$),
      node((1, 0), $A$, "A1"),
    ...
    #commutative-diagram(
      ...
      arr($Z_2$, "A1", $f_2$),
      arr($Z_1$, $Z_2$, $sigma$)
    )] in the _ambient_ category $sans(C)$. Alternatively, morphisms $f_1 -> f_2$ corresponds to those morphisms $sigma: Z_1 -> Z_2$ in $sans(C)$ such that $f_1 = f_2 sigma$.

    Categories constructed in these manners are known as _slice categories_, which are particular cases of _comma categories_.]

#example("Concrete Slice Category")[Suppose $sans(C)$ is the category with $S = ZZ$ and using the relation $<=$. Choose an object $A = 3$ of $sans(C)$. Then the objects of $sans(C)_A$ are morphisms in $sans(C)$ with target $3$, that is, pairs $(n, 3) in ZZ times ZZ$ with $n <= 3$. There is a morphism #align(center)[
    #commutative-diagram(
      node-padding: (30pt, 40pt),
      node((0, 0), $(m, 3)$),
      node((0, 1), $(n, 3)$),
      arr($(m, 3)$, $(n, 3)$, $$)
    )] if and only if $m <= n$. In this example, $sans(C)_A$ may be harmlessly identified with the _subcategory_ of integers $<= 3$ with the _same_ morphisms as in $sans(C)$.]

#example("Co-Slice Category")[We can consider a construction similar to slice categories but one where we take objects to be morphisms in a category $sans(C)$ _from_ a fixed object $A$ to all objects in $sans(C)$. Morphisms are again defined to be suitable commutative diagrams. This construction is known as _the coslice category_.]

#example("Concrete Co-Slice Category")[Let $sans(C) = sans("Set")$ and $A = "fixed singleton" = {star}$. Call the constructed co-slice category $sans("Set")^star$.

An object in $sans("Set")^star$ is then a morphism $f: {star} -> S$ in $sans("Set")$ where $S$ is any set. The information of an object in $sans("Set")^star$ consists of a nonempty set $S$ and an element $s in S$ -- that is, the element $f(star)$. This element determines and is determined by, $f$. So, we can denote objects of $sans("Set")^star$ as pairs $(S, s)$ where $S$ is any set and $s in S$ is any element of $S$.

A morphism between two such objects, $(S, s) -> (T, t)$ corresponds to a set function $sigma: S -> T$ _such that_ $sigma(s) = t$.

Objects of $sans("Set")^star$ are called _pointed sets_.]

#example($sans(C)_(A, B)$)[Start from a category $sans(C)$ and two objects $A, B$ of $sans(C)$. We then define a new category $sans(C)_(A, B)$ by a similar procedure with which we defined $sans(C)_A$.
- $Obj(sans(C)_(A, B)) =$ diagrams \ #box(width: 100%)[#align(center)[
  #commutative-diagram(
  ...
  node((1, 1), $B$),
  arr($Z$, $A$, $f$),
  arr($Z$, $B$, $g$, label-pos: right))]] in $sans(C)$;

- morphisms \ #box(width: 100%)[#align(center)[
  #commutative-diagram(
...
Alternatively, morphisms in $sans(C)_(A, B)$ corresponds to those morphisms $sigma: Z_1 -> Z_2$ in $sans(C)$ such that $f_1 = f_2 sigma$ and $g_1 = g_2 sigma$.]

#example($"Fibered" sans(C)_(A, B)$)[Start with a given category $sans("C")$ and choose two fixed morphisms $alpha: A -> C, beta: B -> C$ in $sans("C")$ with the same target $C$. We can then consider a category $C_(alpha, beta)$ as follows
- $Obj(sans(C)_(alpha, beta))$ = commutative diagrams \ #box(width:100%)[#align(center)[
  #commutative-diagram(
    node-padding: (40pt, 30pt),
    node((0, 0), $Z$),
    node((-1, 1), $A$),
...
-  morphisms correspond to commutative diagrams \ #box(width:100%)[#align(center)[
  #commutative-diagram(
    node-padding: (40pt, 30pt),
    node((0, 0), $Z_1$),
    node((0, 1), $Z_2$),
...
#definition("Isomorphism")[A morphism $f in Hom(C, A, B)$ is an _isomorphism_ if it has a (two-sided) inverse: that is $exists g in Hom(C, B, A)$ such that $ g f = 1_A #h(25pt) f g = 1_B. $]
...
+ If $f in Hom(C, A, B)$ and $g in Hom(C, B, C)$ are isomorphisms, then the composition $g f$ is an isomorphism and $(g f)^(-1) = f^(-1) g^(-1)$.]

#definition[Two objects $A, B$ of a category $sans(C)$ are _isomorphic_ if there is an isomorphism $f:A -> B$.]

#corollary[ Isomorphism is an equivalence relation on the objets of a category. ]

#example[Isomorphisms in $sans("Set")$ are precisely bijective functions.]

#example[In the category $sans(C)$ obtained from the relation $<=$ on $ZZ$ there is a morphism $a -> b$ and $b -> a$ only if $a <= b$ and $b <= a$-- that is, if $ a = b$. So an isomorphism must act from an object to itself and in $sans(C)$ there is only one such object $1_a$.]

#example[There are categories in which every morphism is an isomorphism. These are known as _groupoids_.]

#definition[An _automorphism_ of an object $A$ of a category $sans("C")$ is an isomorphism from $A$ to itself. The set of automorphisms is denoted $"Aut"_(sans("C"))(A)$ and is a subset of $"End"_("C")(A)$.]

#remark[Equipped with composition, $"Aut"_(sans("C"))(A)$ is a group!]

#definition[Let $sans("C")$ be a category. A morphism $f in Hom("C", A, B)$ is a _monomorphism_ if for all objects $Z$ of $sans("C")$ and all morphisms $alpha', alpha'' in Hom(C, Z, A)$, $ f compose alpha' = f compose alpha'' implies alpha' = alpha''. $]

#definition[Let $sans("C")$ be a category. A morphism $f in Hom("C", A, B)$ is an _epimorphism_ if the following holds if for all objects $Z$ of $sans("C")$ and all morphisms $beta', beta'' in Hom("C", B, Z)$, $ beta' compose f = beta'' compose f implies beta' = beta''. $ ]

#example[In $sans("C")$, injective functions are _monomorphisms_ whereas surjective functions are _epimorphisms_.]

#example[In the category $sans("C")$ obtained from the relation $<=$ on $ZZ$, every morphism is both a monomorphism and an epimorphism. However, there is at most one isomorphism between any two pair of objects in $sans("C")$.]

#remark[The previous example shows how the property of $sans("Set")$, wherin a function is an isomorphism iff it is a monomorphism and epimorphism, doesn't generalize to all categories. It can be shown that this is true in _abelian categories_ (but $sans("Set")$ isn't an example of one!) ]

#remark[The property of $sans("Set")$ that a function is an epimorphism iff it has a right inverse doesn't generalize to all categories either.]
